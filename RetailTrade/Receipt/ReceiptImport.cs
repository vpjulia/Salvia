...
namespace RetailTrade.Receipt
{
    ...
    { 
        ...
        {
            DataTable _curTable = (this.bindingSource1.DataSource as DataTable);
           ...
           /*
                ...
                DataRow[] organization = (this.ParentForm as MainForm).mDataSet.Organization.Select("NameFarmTrade=" + orgName);
            ...
            */
            foreach (DataRow _row in _curTable.Rows)
            {
                ...
                int _id = Convert.ToInt32(_idstr);
                //FullDataSet.ProductRow _productRow = (this.ParentForm as MainForm).fullDataSet.Product.FindByID(_id);

                FullDataSet.ProductRow _prRow;
              
                DataRow[] _productRowArr = (this.ParentForm as MainForm).fullDataSet.Product.Select("OldKod="+_id.ToString());
             
                if (_productRowArr.Length==0)
                {
                    _row.RowError = "Не найдена";
                }
                else
                {
                    _prRow = _productRowArr[0] as FullDataSet.ProductRow;
